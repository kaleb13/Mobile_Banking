...
void main() {
  ...
  final transactions = [
    ...
    AppTransaction(name: 'Dashen Bank', sender: 'Nahom', amount: 7000, type: 'income', date: DateTime(2026, 2, 4), category: 'Auto', rawMessage: '', isAutoDetected: true, totalBalance: 39226.94),
    // 2026 Aug 13 Dashen ATM
    AppTransaction(name: 'Dashen Bank', sender: 'ATM / Cash Withdrawal', amount: 5000, type: 'expense', date: DateTime(2026, 8, 13, 14, 15), category: 'Auto', rawMessage: '', isAutoDetected: true, totalBalance: 31119.0),
  ...
  print('Pre-seeded balances before ${actualChartStart.toIso8601String()}: $lastKnownBalance\n');
  ...
  for (int i = 0; i < daysLimit; i++) {
    ...
    print('Day ${i.toString().padLeft(2)} (${d.month}/${d.day}): Bank Total = ${bankTotal.toStringAsFixed(2).padLeft(9)} | Overall Total = ${totalBal.toStringAsFixed(2).padLeft(9)} | Bank Map = $lastKnownBalance');
  }

  print('\nFinal Point in Chart: $latestTotal');
  print('Expected Final Total: 54958.0');
  if ((latestTotal - 54958.0).abs() < 0.01) {
    print('[SUCCESS] Chart calculation produces the exact 54,958.00 ETB total! 🎉');
  } else {
